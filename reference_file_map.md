...
| `create-block-zips.js` | Erzeugt pro Block ein eigenes ZIP in `plugin-zips/` (Standard-Auslieferungsweg) | validiert `block.json` und Build-Dateien; packt Build-Artefakte + `block.json`, `render.php`, `editor.css`, `style.css`, **`jspdf.umd.min.js`** (seit AP-1.1, `PLAN-Summary-PDF-und-Content-Links.md`; die Liste `sourceFiles` gilt für alle Blöcke, ein `existsSync()`-Filter überspringt sie überall dort, wo die Datei nicht liegt – faktisch nur `summary-block`) | `build/blocks/`, `archiver` |
...
| `package.json` | npm-Skripte und Abhängigkeiten | `build`, `start`, `lint:js`, `lint:css`, `block-zips`, `download-libs`, **`download-jspdf`** (seit AP-1.1: lädt jsPDF 2.5.1 nach `blocks/summary-block/jspdf.umd.min.js`; bewusst **nicht** Teil von `download-libs`, weil die Datei anders als die ChemViz-Bibliotheken versioniert ist und nur bei einem Versionswechsel neu geholt werden muss); Version 1.1.8 | Node 16+, npm 7+ |
...
| `summary-block` | `modular-blocks/summary-block` | Summary-Quiz im H5P-Stil mit Gruppen, PDF-Ausgabe | `block.json`, `index.js`, `render.php`, `view.js`, `style.css`, `editor.css`, **`jspdf.umd.min.js`** (seit AP-1.1, `PLAN-Summary-PDF-und-Content-Links.md`) |
...
## Summary-Block: PDF-Export im Detail

Vorhaben „Summary-PDF und Content-Links", Phase 1
(`PLAN-Summary-PDF-und-Content-Links.md`, 2026-09).

**jsPDF liegt lokal im Block-Ordner, nicht in `assets/js/vendor/`** (AP-1.1).
Das ist kein Versehen und keine Abweichung von der ChemViz-Konvention aus
Bequemlichkeit: `create-block-zips.js` packt pro Block ausschließlich
`build/blocks/<slug>/` plus einige Quelldateien **aus dem Block-Ordner**. Eine
Bibliothek unter `assets/js/vendor/` läge außerhalb dieses Zuschnitts und
fehlte in einer per Einzel-Block-ZIP installierten Instanz. Anders als bei
3Dmol.js/Plotly.js gibt es hier **keinen CDN-Rückfall** (AP-1.1 hat ihn
...
block-zips` erst `npm run download-jspdf` ausführen, um kein stillschweigend
kaputtes ZIP zu erzeugen.

| Datei | Zweck | Wichtige Funktionen/Inhalte | Hängt ab von |
|---|---|---|---|
| `blocks/summary-block/jspdf.umd.min.js` | Lokal gebündelte jsPDF-Bibliothek 2.5.1 (UMD-Build) für den PDF-Export des Blocks | Fremdcode, **nicht von Hand bearbeiten**; erneuern ausschließlich über `npm run download-jspdf`. Exponiert den Konstruktor als `window.jspdf.jsPDF`. **Bekannte, harmlose Eigenschaft (AP-1.rev-Befund B4):** Zeile 86 enthält eine ruhende `https://cdnjs.cloudflare.com/…/pdfobject.min.js`-URL in jsPDFs eigenem `output('pdfobjectnewwindow')`-Zweig — der Block ruft ausschließlich `doc.save()` auf, dieser Zweig wird nie erreicht (im Netzwerkmitschnitt bestätigt: kein Aufruf). Kein aktueller DSGVO-Verstoß, nur relevant, falls je ein anderer `output()`-Modus benutzt wird | – (keine) |
| `blocks/summary-block/render.php` | Serverseitiges Rendering | Baut `$summary_data` (als `data-summary`-JSON am Wurzel-`div`) und das Markup. **Seit AP-1.1** zusätzlich ein unbedingtes `wp_enqueue_script('modular-blocks-summary-jspdf', …, MODULAR_BLOCKS_PLUGIN_VERSION, true)` auf die lokale Bibliothek — bewusst unabhängig von `$enable_pdf_download`. **Seit AP-1.3** ermittelt es `$ist_lehrperson` (Theme-Naht, s. u.), baut daraus `$all_statement_texts` und gibt drei neue JSON-Schlüssel aus (`isTeacher`, `teacherPdfCount`, `allStatementTexts`); der Knopf `.teacher-practice-pdf-button` wird **nur bei `$ist_lehrperson`** und nur im Blockkopf (`.summary-teacher-tools`) gerendert, nicht in `.summary-controls`. **Seit AP-1.3 des Nachtrags** (`PLAN-Nachtraege-Summary-PDF-und-Kapitellinks.md`) steht im selben `if`-Zweig zusätzlich `<label class="teacher-pdf-count-label">` + `<input type="number" class="teacher-pdf-count-input">`, vorbelegt mit `$teacher_pdf_count`, `min="1"`, damals `max="count($all_statement_texts)"` (die echte Poolgröße, nicht die Summe der `statements`-Arrays — sonst hätte das Feld Zeilen versprochen, die das PDF nicht liefern kann; **diese Begründung ist seit AP-1.1 des Vorhabens „Summary-Punktesystem, Buttons, Kapitellink-Feinschliff" überholt und unten in derselben Zelle korrigiert** — die `max`-Grenze kommt seither aus `$gesamt_gruppen`, siehe dort, nicht mehr direkt aus der Poolgröße). Feld-`id` aus `$block_id`, damit mehrere Blöcke auf einer Seite eindeutige Label-Bezüge haben. Es wird **nichts gespeichert**: kein AJAX, keine REST-Route, keine Schreibberechtigung (Architekturentscheidung B3). **Seit AP-1.4 des Nachtrags** steht in derselben Zeile ein zweiter Knopf `.teacher-solution-sheet-button` („Lösungsblatt erzeugen") unter derselben Bedingung — er zeigt die richtigen Antworten und darf Lernenden so wenig im HTML begegnen wie der Aussagen-Pool. **Seit AP-1.5 des Nachtrags** ermittelt die Datei zusätzlich `$kapitel_titel` (Titel der obersten Vorfahren-Seite) und gibt ihn als JSON-Schlüssel `kapitelTitel` aus — `is_singular()` als Torwächter, dann `get_post_ancestors()` + `end()`, Wert durch `wp_strip_all_tags()`. **Ohne** `function_exists()`-Brücke zum Theme: `get_post_ancestors()` ist WordPress-Bordmittel (Architekturentscheidung B4), die Funktion braucht das Theme nicht. **Seit AP-1.1 des Vorhabens „Summary-Punktesystem, Buttons, Kapitellink-Feinschliff"** (`PLAN-Summary-Punktesystem-Buttons-und-Kapitellink-Feinschliff.md`) baut `$all_statement_texts` nicht mehr alle Aussagetexte einer Gruppe ein, sondern pro Gruppe **genau die erste als `isCorrect` markierte** Aussage (`break` nach dem ersten Treffer) — jede 3er-Gruppe enthielt vorher drei Formulierungen desselben Sachverhalts, wovon jetzt nur noch die richtige in den Pool kommt. Fehlt einer Gruppe eine `isCorrect`-Aussage (Datenfehler im Editor), wird sie ohne Fehler übersprungen (kein Pool-Eintrag). Der JSON-Schlüssel heißt weiterhin `allStatementTexts` (Plan erlaubt das ausdrücklich), enthält aber nur noch einen Eintrag je Gruppe. Die `max`-Grenze des Zahlenfelds kommt seither aus der neuen Variable `$gesamt_gruppen = count($groups_data)` (Gruppenzahl) statt der früheren Gesamtzahl aller Einzelaussagen; das `value`-Attribut wird zusätzlich auf `min($teacher_pdf_count, $gesamt_gruppen)` geklemmt — behebt den bekannten Bug G3 (Nachtragsplan), bei dem ein gespeichertes `teacherPdfCount` größer als die tatsächliche Gruppenzahl das Feld mit `value > max` (HTML-technisch ungültig) öffnen konnte. **Seit AP-1.2 desselben Vorhabens** ist der Text hinter dem Schlüssel `strings.score` (Anzeige `.score-display`) von `__('Punkte', …)` auf `__('Aussagensätzen richtig', …)` geändert — Grund: Das Punktemodell zählt seither Gruppen (Aussagen-Trios) statt Einzelaussagen, `view.js` baut daraus `"${finalScore} von ${groups.length} ${strings.score} (…%)"`. Schlüsselname bewusst unverändert (`view.js` liest weiterhin `strings.score`). **Seit AP-1.4 desselben Vorhabens** (Button-Styling): `$button_style` (primäre Buttons `.continue-button`/`.pdf-download-button`/`.solution-button`) und `$button_secondary_style` (sekundäre Buttons `.retry-button`/`.teacher-practice-pdf-button`/`.teacher-solution-sheet-button`) geändert — `border-radius` in beiden von `4px` auf `6px` (Projektstandard, siehe `statement-summary`; MUSS hier UND in `style.css` stehen, da der Inline-Style die externe Regel überschreibt); `$button_style`s `color: #fff` auf `color: var(--color-text-on-accent, #ffffff)`; `$button_secondary_style`s `background: transparent` ersatzlos entfernt (die neue helle Flächenfarbe kommt jetzt ausschließlich aus den `!important`-Regeln in `style.css`, ein weiterhin gesetztes `transparent` wäre zwar wirkungslos, aber irreführender toter Code gewesen) | `block.json`-Attribute, `jspdf.umd.min.js`, optional Theme-Funktion `simple_clean_ist_lehrperson()` |
| `blocks/summary-block/block.json` | Metadaten und Attribute | **Seit AP-1.3** zusätzlich `teacherPdfCount` (`number`, Vorgabe 10). **Seit AP-1.1 des Nachtrags** (`PLAN-Nachtraege-Summary-PDF-und-Kapitellinks.md`) steht `pdfDownloadThreshold` auf `"default": 0` statt bisher `100` — der Schüler-Download erscheint damit nach jedem Ergebnis, auch bei 0 %. **Wirkt rückwirkend auf Bestandsseiten:** Gutenberg serialisiert Attribute nicht, deren Wert dem `block.json`-Default entspricht; keine der 26 Bestandsinstanzen auf dem Testserver trägt `pdfDownloadThreshold` im Blockkommentar, alle übernehmen deshalb den neuen Standard. Wer für eine einzelne Seite doch eine Mindestpunktzahl will, setzt sie im Inspector-Regler „PDF-Download ab (%)" (dann wird der Wert serialisiert und gewinnt). **`version` seit AP-1.doc des Vorhabens „Summary-Punktesystem, Buttons, Kapitellink-Feinschliff" auf `2.2.3`** (zuvor `2.2.2`, über zwei Phasen mit sichtbaren Stylesheet-Änderungen unverändert geblieben — Review-Befund M1: `register_block_style_handle()` nutzt dieses Feld als `?ver=`-Cache-Buster, sobald `style-index.css` NICHT inline eingebettet wird, siehe `class-block-manager.php`/`wp_maybe_inline_styles()`-Budget 20 000 Byte; ohne die Erhöhung hätten wiederkehrende Produktiv-Besucher das neue Button-Styling aus AP-1.4 aus dem Browser-Cache weiterhin mit den alten Werten gesehen) | – |
| `blocks/summary-block/index.js` | Editor-Registrierung | **Seit AP-1.3** ein `RangeControl` „Anzahl Aussagen im Übungs-PDF" (1–50) im Panel neben `pdfDownloadThreshold`. Bewusst **nicht** hinter `{enablePdfDownload && …}` wie die zwei Regler darüber: Der Lehrer-Knopf hängt nicht am Schüler-PDF-Download und wäre sonst unkonfigurierbar, sobald jemand den Schüler-Download abschaltet. **Seit AP-1.2 des Vorhabens „Summary-Punktesystem, Buttons, Kapitellink-Feinschliff"** ist der `RangeControl` „Punktabzug pro Fehler" (Attribut `penaltyPerWrongAnswer`) entfernt — das Punktemodell zählt seither gruppenbasiert (ein Punkt je Aussagen-Trio, verloren beim ersten Fehlklick), ein Abzug pro Fehlklick passt nicht mehr dazu. Das Attribut selbst bleibt in `block.json`/den destrukturierten `attributes` **nicht** mehr referenziert, aber im Datenmodell erhalten (Architekturentscheidung C3, Kompatibilität mit bereits gespeicherten Blockinstanzen) | `block.json` |
| `blocks/summary-block/style.css` | Frontend-Styling | **Seit AP-1.3** teilt sich `.teacher-practice-pdf-button` die Umriss-Optik von `.retry-button` (`var(--sb-primary, #e24614)`); ohne diese Regel bliebe der Knopf ungefärbt, weil `$button_secondary_style` in `render.php` zwar `border-style`, aber keine `border-color` setzt. Dazu `.summary-teacher-tools` (nur Abstand). **Seit AP-1.2 des Nachtrags** zusätzlich `.summary-wrong-count` (Fußnotenzeile in der Lösungsanzeige): gedämpft-kursiv über `var(--summary-text-light)`, bei `:not([data-wrong-count="0"])` stattdessen `var(--summary-danger)` und halbfett — so sind fehlerfreie Aussagensätze auf einen Blick von fehlerbehafteten zu unterscheiden. **Seit AP-1.3 des Nachtrags** ist `.summary-teacher-tools` von `block` auf `inline-flex` mit `flex-wrap`/`gap` umgestellt (Label + Zahlenfeld + Knopf als eine Zeile, die auf schmalen Bildschirmen umbricht), dazu `.teacher-pdf-count-label` und `.teacher-pdf-count-input`. **Seit AP-1.4 des Nachtrags** steht `.teacher-solution-sheet-button` in derselben Umriss-Regel wie `.teacher-practice-pdf-button` (Grund- und Hover-Zustand). **Seit AP-1.4 des Vorhabens „Summary-Punktesystem, Buttons, Kapitellink-Feinschliff" (Architekturentscheidung C4) — die bisherige Umriss-Optik der Sekundär-Buttons ist damit wieder entfallen, ersetzt durch das folgende Muster:** (1) `--summary-primary`-Fallback (Zeile 26) von `#1a73e8` (totes Google-Blau) auf `#e24614` korrigiert; (2) Fokusring von `.statement-option:focus` (vormals `rgba(26, 115, 232, 0.1)`) auf `rgba(226, 70, 20, 0.1)` (Dezimalzerlegung von `#e24614`); (3) `.summary-button`-Basisregel `border-radius` von `8px` auf `6px` (faktisch wirkungslos, siehe `render.php`-Zeile oben — nur Konsistenz für den Fall, dass die Inline-Erzeugung künftig entfällt); (4) `.continue-button`/`.pdf-download-button`/`.solution-button`: `color: #fff !important` auf `color: var(--color-text-on-accent, #ffffff) !important`; (5) `.retry-button`/`.teacher-practice-pdf-button`/`.teacher-solution-sheet-button`: neue `background`/`background-color` (`var(--color-background-light, #f8f9fa) !important`, hover `var(--color-ui-surface-light, #f5ede9) !important`) ergänzt das bisherige `border-color`/`color` (unverändert `--sb-primary`/`--sb-primary-hover`) — Muster „helle Fläche + farbiger Rand" von `statement-summary/style.css:184-193` übernommen, aber mit den lokalen, bereits Customizer-gekoppelten `--sb-primary`/`--sb-primary-hover`-Tokens statt der dortigen `--color-ui-surface`-Variablen, um die bestehende Kopplung nicht zu verdoppeln. `--color-background-light`/`--color-ui-surface-light` sind beide bereits dark-mode-gekoppelt (Theme/style.css) — im Darkmode ergibt das eine dunkle Fläche mit weiterhin farbigem Rand/Text, live im Browser (Testserver, echte 25-Gruppen-Produktivseite, `data-theme="dark"` gesetzt) als kontrastreich bestätigt, keine neue `[data-theme="dark"]`-Regel nötig. Voraussetzung dafür: `render.php`s `$button_secondary_style` darf `background` nicht mehr inline setzen (siehe dortige Zeile) | `render.php`-Wrapper-Properties `--sb-primary`/`--sb-primary-hover`, Block-Variablen `--summary-text-light`/`--summary-danger`, Theme-Variablen `--color-text-on-accent`/`--color-background-light`/`--color-ui-surface-light` |
| `blocks/summary-block/view.js` (AP-1.1, `PLAN-Summary-Punktesystem-Buttons-und-Kapitellink-Feinschliff.md`) | Übungsblatt-Pool-Absicherung | `generateTeacherPracticePDF()`: unmittelbar nach dem Kopieren von `allStatementTexts` in `pool` zusätzlich `pool = Array.from(new Set(pool))` — reine Vorsichtsmaßnahme, nicht weil aktuell nötig (`render.php` liefert seit diesem AP bereits nur noch eine Aussage je Gruppe), sondern als Schutz gegen künftige Datenfehler, falls zwei Gruppen zufällig wortgleiche Texte tragen sollten. Fisher-Yates-Shuffle und die anschließende `Math.min(…, pool.length)`-Deckelung bleiben strukturell unverändert. `N` bezieht sich seit diesem AP auf die Gruppenzahl, nicht mehr auf die Gesamtzahl aller Einzelaussagen (siehe `render.php`-Zeile oben) | `render.php`-JSON-Schlüssel `allStatementTexts` (jetzt gruppenbasiert) |
| `blocks/summary-block/view.js` (AP-1.2, `PLAN-Summary-Punktesystem-Buttons-und-Kapitellink-Feinschliff.md`) | Punktesystem auf Aussagen-Trio umgestellt | `calculateFinalScore()` neu implementiert: zählt jetzt, wie viele Gruppen `!wrongAttemptsByGroup[index]` erfüllen (kein Fehlklick in dieser Gruppe) — ein Punkt je Gruppe, verloren beim ERSTEN Fehlklick, unabhängig von der Gesamtzahl weiterer Fehlklicks in derselben Gruppe. Nutzt die bereits vorhandene `wrongAttemptsByGroup`-Struktur (siehe AP-1.2 des Nachtrags, Zeile unten) — keine neue Zählstruktur (Architekturentscheidung C2). Die frühere Fallunterscheidung Regelmodus (`Math.max(0, score)`) vs. `deferredFeedback` (Alles-oder-nichts pro Block gegen `allSelections`) ist vollständig entfallen: **beide Modi laufen jetzt über denselben Code**, `deferredFeedback` wird in `calculateFinalScore()` nicht mehr abgefragt. `showResults()`: `percentage` bezieht sich jetzt auf `groups.length` statt `totalCorrect` (Gesamtzahl aller `isCorrect`-Einzelaussagen — im Regelfall identisch mit der Gruppenzahl, aber nicht bei Gruppen mit `correctCount !== 1`); `.score-display`-Text lautet jetzt `"${finalScore} von ${groups.length} ${strings.score} (…%)"` (siehe `render.php`-Zeile zu `strings.score` oben) statt `"${finalScore}/${totalCorrect} Punkte (…%)"`. `pdfDownloadThreshold`-Vergleich, Icon-/Text-Schwellen (`successText`/`partialSuccessText`/`failText`) und `generatePDF()`s `Ergebnis: N% richtig`-Zeile (liest `lastPercentage`) brauchten **keine Codeänderung** — sie lesen alle denselben `percentage`-Wert und übernehmen das neue Modell automatisch. Die alte, aussagenbasierte `score`/`wrongAttempts`/`penaltyPerWrong`-Buchführung (Deklaration, `resetQuiz()`, Abzug bei Fehlklick) bleibt im Code stehen, wird aber von `calculateFinalScore()` nicht mehr gelesen — bewusst unangetastet gelassen (außerhalb des AP-Scopes, `penaltyPerWrongAnswer` bleibt laut Architekturentscheidung C3 im Datenmodell). Getestet über jsdom-Harness (führt das echte `view.js` in einem aus dem echten `render.php` erzeugten DOM aus, siehe Übergabenotiz AP-1.2) und live im Browser auf einer Produktivseite mit 25 Gruppen (3 gezielte Fehlklicks → „22 von 25 Aussagensätzen richtig (88%)") | `render.php`-JSON-Schlüssel `groups`, `wrongAttemptsByGroup` (Struktur aus AP-1.2 des Nachtrags) |
| `blocks/summary-block/view.js` (AP-1.3, `PLAN-Summary-Punktesystem-Buttons-und-Kapitellink-Feinschliff.md`) | `progressiveReveal:false` erreicht jetzt `showResults()` | Neue Hilfsfunktion `allGroupsCompleted()` (direkt nach `isGroupCompleted()`): prüft `Array.prototype.every.call(groupElements, isGroupCompleted)`. Grund: Bei `progressiveReveal:false` sind laut `resetQuiz()` von Anfang an ALLE Gruppen gleichzeitig sichtbar/bearbeitbar (`groupEl.style.display = progressiveReveal && index > 0 ? 'none' : 'block';`) — `currentGroupIndex` bleibt in diesem Modus dauerhaft `0`, weil `goToNextGroup()` (der einzige Ort, der ihn erhöht) hier nie aufgerufen wird, weder über den Auto-Advance-Pfad im Regelmodus noch über den Weiter-Knopf (der nur bei `progressiveReveal:true` via `showContinueButton()` erscheint). „Letzte Gruppe nach Index" ist deshalb kein taugliches Abschlusskriterium für `progressiveReveal:false`. In `handleStatementClick()` (Regelmodus-Zweig, nicht `deferredFeedback`): Der bestehende `if (progressiveReveal) { setTimeout(…) }`-Block, der bei Gruppenabschluss `goToNextGroup()`/`showResults()` auslöst, bleibt **unverändert** (Regressionsschutz AK2) — neu ist ein `else if (allGroupsCompleted())`-Zweig, der bei `progressiveReveal:false` nach Abschluss der zuletzt bearbeiteten Gruppe prüft, ob JEDE Gruppe fertig ist, und dann `setTimeout(showResults, 800)` auslöst. Vorher fehlte dieser gesamte Pfad ersatzlos — `showResults()` (und damit Ergebnisanzeige, PDF-/Wiederholen-Buttons) war bei `progressiveReveal:false` nie erreichbar. Der bereits vorbestehende, hier bewusst **nicht** angefasste `deferredFeedback`-Mangel (Nicht-Ziel dieses Plans) ist von dieser Änderung unberührt: `isGroupCompleted()` prüft im `deferredFeedback`-Zweig weiterhin auf `.correct`/`.incorrect`-Klassen, die dort nie gesetzt werden — `allGroupsCompleted()` liefert für diesen Modus also weiterhin `false`, das ist kein neuer Fehler. Getestet über jsdom-Harness (führt das echte `view.js` in einem aus dem echten `render.php` erzeugten DOM aus; 19/19 Prüfungen bestanden, inkl. Reihenfolgen-Unabhängigkeit — Gruppe 1 vor Gruppe 0 beantwortet — und AP-1.2-Punktemodell in beiden Modi); Gegenprobe mit gestashtem Vorzustand bestätigt, dass genau die `progressiveReveal:false`-Tests vor der Änderung fehlschlugen, die `progressiveReveal:true`-Tests dagegen unverändert bestanden | `isGroupCompleted()`, `groupElements`, `showResults()` |
| `blocks/summary-block/view.js` (AP-1.5 des Nachtrags, `PLAN-Nachtraege-Summary-PDF-und-Kapitellinks.md`) | Kapitelzeile in allen drei PDFs | Neue Hilfsfunktion `writeChapterLine(doc, startY, margin, maxWidth, lineHeight)`, aufgerufen von `generatePDF()`, `generateTeacherPracticePDF()` und `generateSolutionSheetPDF()`. Schreibt den Wert `kapitelTitel` aus der `data-summary`-JSON als graue 11-pt-Zeile **über** den bestehenden Blocktitel und liefert die neue y-Position zurück. Ist `kapitelTitel` leer, kehrt sie sofort zurück, **ohne** Dokumentzustand oder y-Position zu verändern — die drei Köpfe stehen dann Zeile für Zeile exakt dort, wo sie vor diesem AP standen (nachgemessen: Ergebnis-PDF 20/35/49, Übungsblatt 20/32/46). Der Wert läuft durch `htmlToText()`, weil er aus der JSON stammt und HTML-Entities enthalten kann | `render.php`-JSON-Schlüssel `kapitelTitel`, `htmlToText()` |
| `blocks/summary-block/view.js` (AP-1.4 des Nachtrags, `PLAN-Nachtraege-Summary-PDF-und-Kapitellinks.md`) | Lösungsblatt-PDF | Neue Funktion `generateSolutionSheetPDF()`, Dokument-Setup analog `generateTeacherPracticePDF()`, Inhalt bewusst dessen Gegenstück: **alle** Aussagen **aller** Gruppen, gegliedert nach „Frage N von M", je Aussage `[RICHTIG]`/`[FALSCH]` nach `isCorrect` (ASCII wegen WinAnsiEncoding), **keine** Zufallsauswahl, **keine** Deckelung durch das Zahlenfeld aus AP-1.3, **kein** Prozentwert (kein Bezug zu einem Schüler-Durchlauf). Dateiname `loesungsblatt_<ts>.pdf`. Datenquelle ist die Konstante `groups` aus der `data-summary`-JSON — `allStatementTexts` taugt hier nicht, weil diese Liste bewusst kein `isCorrect` trägt. Click-Listener nur, wenn der Knopf im DOM steht | `render.php`-Knopf `.teacher-solution-sheet-button`, `render.php`-JSON `groups`, jsPDF |
| `blocks/summary-block/view.js` (AP-1.2 des Nachtrags, `PLAN-Nachtraege-Summary-PDF-und-Kapitellinks.md`) | Falsch-Zähler je Aussagensatz | Neue Blockinstanz-Variable `wrongAttemptsByGroup` (Objekt Gruppenindex → Anzahl), bewusst **neben** dem bestehenden globalen `wrongAttempts` statt an dessen Stelle (Architekturentscheidung B2: `wrongAttempts` hängt an `penaltyPerWrong`, der neue Zähler ist rein informativ). Gefüllt von `countWrongAttempt(groupEl)` an **beiden** Fehlklick-Stellen in `handleStatementClick()` — im Regelmodus **und** im `deferredFeedback`-Zweig, wo bis dahin überhaupt nicht gezählt wurde. Schlüssel ist `groupEl.dataset.groupIndex`, **nicht** `currentGroupIndex`: bei `progressiveReveal: false` stehen alle Gruppen offen und die Klickreihenfolge ist frei. Zurückgesetzt in `resetQuiz()`. Ausgabe über `addWrongCountLine()` in `showSolution()`, je Gruppe genau eine Zeile — auch bei 0. Die Zeile trägt bewusst die Klasse `.summary-wrong-count` und **nicht** `.summary-item`: `updateSummaryAfterEvaluation()` läuft im verzögerten Feedback über alle `.summary-item` und greift dort auf `.summary-bullet`/`.summary-text` zu, die diese Zeile nicht hat | `render.php`-Attribut `data-group-index`, `style.css`-Regel `.summary-wrong-count` |
| `blocks/summary-block/view.js` (AP-1.3) | Übungs-PDF | Neue Funktion `generateTeacherPracticePDF()`: Fisher-Yates auf einer Kopie von `allStatementTexts`, dann `min(teacherPdfCount, Poolgröße)` Einträge; Titel „Übungsblatt", je Aussage eine Ankreuzzeile `[  ] Richtig     [  ] Falsch`, **kein** Richtig/Falsch, **kein** Prozentwert; eigener Dateiname `uebungsblatt_<ts>.pdf`. Click-Listener nur, wenn der Knopf im DOM steht — eine zweite, client-seitige Rechteprüfung gibt es bewusst nicht. **Seit AP-1.3 des Nachtrags** liest die Funktion die Anzahl aus `teacherPdfCountInput` (`block.querySelector('.teacher-pdf-count-input')` — bewusst blockweit statt `document.querySelector`, sonst zöge jeder Block auf einer Seite mit mehreren summary-blocks den Wert des ersten Feldes) und fällt bei fehlendem/leerem/unlesbarem Feld auf das Attribut `teacherPdfCount` zurück; die Deckelung `Math.min(…, pool.length)` ist unverändert die aus dem Vorgängerplan | `render.php`-JSON, `render.php`-Feld `.teacher-pdf-count-input`, jsPDF |
...
| `blocks/summary-block/view.js` (AP-1.fix1, `PLAN-Summary-Punktesystem-Buttons-und-Kapitellink-Feinschliff.md`) | „Wiederholen" setzt Klickbarkeit aller Aussagen zurück (Korrektur-AP, von AP-1.rev gefundener, vorbestehender Mangel M2) | `resetQuiz()` entfernte bislang nur die native `disabled`-**Property** (`stmtEl.disabled = false`), nicht aber die gleichnamige CSS-**Klasse** `disabled` (`style.css:262-267`, `pointer-events: none; opacity: 0.5`), die `handleStatementClick()` bei Gruppenabschluss auf die übrigen (nicht angeklickten) Aussagen dieser Gruppe setzt. Nach „Wiederholen" blieben dadurch die meisten Aussagen-Buttons dauerhaft unklickbar — je Gruppe nur die zuvor richtige Aussage anklickbar, ein zweiter Durchlauf ergab zwangsläufig 100%. Fix: dieselbe `classList.remove(...)`-Anweisung, die bereits `correct`/`incorrect`/`solution-correct`/`solution-incorrect` entfernt, um `disabled` **und** `selected` (Klasse des `deferredFeedback`-Zweigs, `style.css:270-274`, ebenfalls `pointer-events: none`) ergänzt — keine neue Schleife, dieselbe bestehende `statements.forEach(...)` in `resetQuiz()`. Betrifft **beide** `progressiveReveal`-Modi (die Schleife läuft unabhängig vom Modus über alle Gruppen) und ist unabhängig vom `deferredFeedback`-Attribut. Live im echten Browser verifiziert (`getComputedStyle()`, temporäre Testseite mit der tatsächlich deployten `view.js`/`style-index.css`): vor dem Fix blieben nach „Wiederholen" alle zuvor deaktivierten `.statement-option` bei `pointerEvents: 'none'`; nach dem Fix `pointerEvents: 'auto'` bei allen 9 Aussagen eines 3-Gruppen-Testblocks, in **beiden** Modi. Ein zweiter Durchlauf mit echten Mausklicks (nicht nur `.click()`) auf genau die vorher „steckenbleibenden" Aussagen liefert danach ein korrekt abweichendes Ergebnis (33%, nicht mehr pauschal 100%). Der Mangel war vorbestehend (identisch auf `main` vor diesem Fix) und nicht durch Phase 1 verursacht — nur durch AP-1.3 (progressiveReveal:false erreicht jetzt eine zweite Ergebnisanzeige) erstmals beobachtbar geworden | `style.css`-Regeln `.statement-option.disabled`/`.statement-option.selected` |
