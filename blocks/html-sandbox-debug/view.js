/**
 * HTML Sandbox DEBUG Block - Frontend Script
...
(function () {
	...
	/**
	 * Initialize all HTML Sandbox DEBUG blocks on the page
	 */
	function initHtmlSandbox() {
		const blocks = document.querySelectorAll('.wp-block-modular-blocks-html-sandbox-debug');
		...
		const observer = new MutationObserver((mutations) => {
			...
			mutations.forEach((mutation) => {
				mutation.addedNodes.forEach((node) => {
					if (
						node.nodeType === 1 &&
						(node.classList?.contains('wp-block-modular-blocks-html-sandbox-debug') ||
							node.querySelector?.('.wp-block-modular-blocks-html-sandbox-debug'))
					) {
						shouldInit = true;
					}
				});
