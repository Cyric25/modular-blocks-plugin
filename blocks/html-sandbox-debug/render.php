...
// TEMPORARY DEBUG VERSION - CAPABILITY CHECK DISABLED!
// This is for testing only. Re-enable capability check after debugging.

// Show debug info about current user
$current_user = wp_get_current_user();
$debug_info = sprintf(
	'DEBUG: User=%s, Roles=%s, edit_posts=%s, unfiltered_html=%s',
	$current_user->user_login,
	implode(',', $current_user->roles),
	current_user_can('edit_posts') ? 'YES' : 'NO',
	current_user_can('unfiltered_html') ? 'YES' : 'NO'
);
error_log('HTML Sandbox Block: ' . $debug_info);
...
// Wrapper classes - Use debug block name
$wrapper_classes = ['wp-block-modular-blocks-html-sandbox-debug'];
